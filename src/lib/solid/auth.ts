...
const RETURN_TO_KEY = "mind-calendar:return-to";
 ...
 * an infinite /login/callback ↔ /calendar loop, and even in the happy path it
 ...
 * the attempted path so reconnecting returns there.
 */
...
/**
 * True when Calendar is running inside another origin's frame (e.g. the Mind
 ...
 * (header chip, page body, callback page), so memoizing the call to a
...
export async function ensureSession(): Promise<ISessionInfo> {
  ...
  // embed / standalone), keeping Calendar unchanged outside the shell.
  if (isEmbedded()) {
