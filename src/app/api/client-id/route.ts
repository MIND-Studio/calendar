...
/**
 * Solid-OIDC **Client Identifier Document** for Mind Calendar.
 *
 ...
 * ConnectForm). Origin is derived from the request so the same code serves
 * the right URLs in every deploy. NOTE: a containerised dev IdP can't
 * dereference a `localhost` URL, so the client only uses this on non-local
 * origins — see `startLogin` in ConnectForm.
 *
 * This route persists nothing: it is a static identity document, not state.
 */
