 ...
 * `/api/client-id` document) so the IdP recognises the same client across
 * sessions and stops re-prompting for consent on every load. Gated to
 * non-localhost origins: a containerised dev IdP can't dereference a
 * `http://localhost:3140/...` client id document, so dev keeps the
    ...
    return (
      <div className="rounded-lg border border-primary/40 bg-primary/5 p-5">
        ...
        <div className="mt-4 flex gap-3">
          <Button asChild>
            <a href="/calendar">Open my calendar →</a>
          </Button>
    ...
    <>
      <MindLoginCard
        ...
        onLogin={async ({ issuer }) => {
          ...
          rememberReturnToDefault("/calendar");
